...
- **Rediseño mayor (`Docs/MP.md`) — `/app`**: 14/14 escenas de la Fase 2 completas y verificadas, más sistema visual de fondos por escena (3 tratamientos: retrato/cadena/lista/base, `SceneBackdrop.tsx`) implementado tras propuesta antes/después confirmada por el usuario. **Gate de aprobación humana pendiente antes de Fase 3** (3D real con Three.js, imágenes finales, empaquetado Tauri) — no iniciar sin confirmación explícita del usuario. Ver `app/AGENTS.md` para detalle completo y qué sigue (documentado para que otro agente, ej. OpenCode, pueda continuar sin perder contexto).
