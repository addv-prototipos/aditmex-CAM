...
Nada activo ahora mismo — se acaba de cerrar el segmento de verificación visual de los mocks 3D de `/app`. Esperando decisión del usuario en el **gate de aprobación humana** (MP.md §0.9) antes de avanzar a Fase 3.

## Falta / pendiente

- **Docker**: el protocolo exige "Docker siempre" pero el sitio estático raíz no tiene contenedorización todavía. No implementado.
- **Rediseño mayor (`Docs/MP.md`) — `/app`**: 14/14 escenas de la Fase 2 completas y verificadas, sistema visual de fondos por escena (4 tratamientos: retrato/cadena/lista/red/base, `SceneBackdrop.tsx`), y mocks de los 3 momentos 3D de MP.md §21 (partículas organizándose, cadena en secuencia, red con pulso) — **verificados visualmente en navegador** (extensión Claude-in-Chrome reconectó tras el bloqueo de sesiones previas). **Gate de aprobación humana pendiente antes de Fase 3** (3D real con Three.js, imágenes finales, empaquetado Tauri) — no iniciar sin confirmación explícita del usuario. Ver `app/AGENTS.md` para detalle completo y qué sigue (documentado para que otro agente, ej. OpenCode, pueda continuar sin perder contexto).
